...
func (d *UserDesc) SwaggerDef(options ...swagger.Option) (result []byte) {
	if len(options) > 0 || len(d.opts.SwaggerDefaultOpts) > 0 {
		var err error
		var s = &spec.Swagger{}
		if err = s.UnmarshalJSON(_swaggerDef_api_user_proto); err != nil {
		...
		result = _swaggerDef_api_user_proto
	}
	return result
}
...
func (d *UserDesc) RegisterHTTP(mux transport.Router) {
	...
	{
		// Handler for GetUsersByIDs, binding: POST /user/get-users-by-ids
		var h http.HandlerFunc
		h = http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			defer r.Body.Close()

			unmFunc := unmarshaler_goclay_User_GetUsersByIDs_0(r)
			rsp, err := _User_GetUsersByIDs_Handler(d.svc, r.Context(), unmFunc, d.opts.UnaryInterceptor)
		...
		if isChi {
			chiMux.Method("POST", pattern_goclay_User_GetUsersByIDs_0, h)
		} else {
			mux.Handle(pattern_goclay_User_GetUsersByIDs_0, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.Method != "POST" {
...
func (c *User_httpClient) GetUsersByIDs(ctx context.Context, in *GetUsersByIDsRequest, opts ...grpc.CallOption) (*GetUsersByIDsResponse, error) {
	mw, err := httpclient.NewMiddlewareGRPC(opts)
	if err != nil {
		return nil, err
	}

	path := pattern_goclay_User_GetUsersByIDs_0_builder(in)
...
// patterns for User
var (
	pattern_goclay_User_GetUsersByIDs_0 = "/user/get-users-by-ids"

	pattern_goclay_User_GetUsersByIDs_0_builder = func(in *GetUsersByIDsRequest) string {
		values := url.Values{}

		u := url.URL{
			Path:     fmt.Sprintf("/user/get-users-by-ids"),
			RawQuery: values.Encode(),
		}
		return u.String()
	}

	unmarshaler_goclay_User_GetUsersByIDs_0_boundParams = &utilities.DoubleArray{Encoding: map[string]int{"": 0}, Base: []int{1, 1, 0}, Check: []int{0, 1, 2}}
)

// marshalers for User
var (
	unmarshaler_goclay_User_GetUsersByIDs_0 = func(r *http.Request) func(interface{}) error {
		return func(rif interface{}) error {
			req := rif.(*GetUsersByIDsRequest)

			if err := errors.Wrap(runtime.PopulateQueryParameters(req, r.URL.Query(), unmarshaler_goclay_User_GetUsersByIDs_0_boundParams), "couldn't populate query parameters"); err != nil {
...
var _swaggerDef_api_user_proto = []byte(`{
  "swagger": "2.0",
  "info": {
  ...
  "paths": {
    "/user/get-users-by-ids": {
      "post": {
        "summary": "Получить пользователей по идентификаторам",
        "operationId": "GetUsersByIDs",
        "responses": {
          "200": {
            "description": "A successful response.",
            "schema": {
              "$ref": "#/definitions/userGetUsersByIDsResponse"
            }
          }
        },
        "parameters": [
          {
            ...
            "required": true,
            "schema": {
              "$ref": "#/definitions/userGetUsersByIDsRequest"
            }
          }
        ],
        "tags": [
  ...
  "definitions": {
    ...
    "userGetUsersByIDsResponse": {
      "type": "object",
      "properties": {
        "users": {
          "type": "array",
          "items": {
            "$ref": "#/definitions/GetUsersByIDsResponseUser"
          }
        }
      },
      "title": "GetUsersByIDsResponse"
    }
  }
}
